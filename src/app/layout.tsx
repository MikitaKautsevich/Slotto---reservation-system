...
function Header() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [showLogoutPopup, setShowLogoutPopup] = useState(false); // состояние попапа
  const { user } = useAuth();
  const router = useRouter();
  ...
  const handleSignOut = () => {
    ...
    signOut(auth)
      ...
      .catch(console.log)
      .finally(() => setShowLogoutPopup(false));
  };
      ...
      {menuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-lg border-t border-gray-200 shadow-lg">
          ...
          <button
            ...
            className="block mx-8 my-4 w-[calc(100%-2rem)] px-6 py-3 bg-gradient-to-r from-red-500 to-rose-600 text-white text-center rounded-xl shadow-md hover:shadow-xl transition-all duration-300"
          >
            Log out
          </button>
        </div>
